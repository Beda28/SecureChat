...
// export const AddServer = ({op, setop}: {op: number, setop: React.Dispatch<React.SetStateAction<number>>}) => {
//     const [servername, setservername] = useState('');
//     const [resultmessage, setresultmessage] = useState('');
//     const [active, setactive] = useState<boolean>(false);
//     const {setCurrentChatServer} = useAppContext()
//     const navigate = useNavigate()

//     useEffect(() => {
//         if (op != 2) setresultmessage('')
//     }, [op])
...
//             const res = await axios.post('/api/server/add', {
//                 user_uuid: getId('uuid'),
//                 server_name: servername
//             })
...
//                 <AddUIBox op={op} onClick={clickcheck}></AddUIBox>
//                 <InviteServerList op={op} onClick={(e) => {e.preventDefault()}}>
//                     <InviteTitle>서버 생성</InviteTitle>
//                     <ChatInput placeholder="서버 이름을 입력해주세요" onKeyDown={(e) => {enterkey(e)}} onChange={(e) => {setservername(e.target.value)}} value={servername}></ChatInput>
//                     <ResultMessage active={active} op={resultmessage != ''}>{resultmessage}</ResultMessage>
//                 </InviteServerList>
//             </BackgroundC>
//         </>
...
// export const InviteServerUser = ({op, setop, servername, setservername}: 
//     {op: number, setop: React.Dispatch<React.SetStateAction<number>>, servername: string, setservername: React.Dispatch<React.SetStateAction<string>>}) => {
//     const [friendlist, setfriendlist] = useState<string[]>([]);
//     const [resultmessage, setresultmessage] = useState('');
//     const [active, setactive] = useState<boolean>(false);
...
//                 <AddUIBox op={op} onClick={clickcheck}></AddUIBox>
//                 <InviteServerList op={op} onClick={(e) => {e.preventDefault()}}>
//                     <InviteTitle>친구 초대하기</InviteTitle>
//                     <AddServerFriendList>
//                         <AddServerUserList>{userlist()}</AddServerUserList>
//                     </AddServerFriendList>
//                     <ResultMessage active={active} op={resultmessage != ''}>{resultmessage}</ResultMessage>
